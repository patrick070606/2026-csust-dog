...
/* 足端轨迹整体偏转角（度）：四条腿统一旋转足端轨迹坐标系。默认 0 表示不旋转。 */
#define DOG_GAIT_DEFAULT_BIAS_ANGLE_DEG    0.0f
...
#define DOG_GAIT_STAND_FOOT_Y_LF_MM             (DOG_GAIT_DEFAULT_L1_MM + DOG_GAIT_DEFAULT_L2_MM - 160.0f)
#define DOG_GAIT_STAND_FOOT_Y_RF_MM             (DOG_GAIT_DEFAULT_L1_MM + DOG_GAIT_DEFAULT_L2_MM - 160.0f)
#define DOG_GAIT_STAND_FOOT_Y_LB_MM             (DOG_GAIT_DEFAULT_L1_MM + DOG_GAIT_DEFAULT_L2_MM - 160.0f)
#define DOG_GAIT_STAND_FOOT_Y_RB_MM             (DOG_GAIT_DEFAULT_L1_MM + DOG_GAIT_DEFAULT_L2_MM - 160.0f)
...
#define DOG_GAIT_WALK_BODY_MAX_STEP_MM           12.0f // 单次步态更新允许的最大重心移动量，防止目标变化时足端坐标突跳。
...
#define DOG_GAIT_WALK_RB_LEFT_FRONT_PRELOAD_MM   -5.0f // 在 LB 起摆前施加到 LF 的左侧预加载量，并保持至 RB 落脚。
#define DOG_GAIT_WALK_RB_LEFT_REAR_PRELOAD_MM    -10.0f // LB 起摆前开始施加到 LB，并连续保持至 RB 落脚后的预加载量。
#define DOG_GAIT_WALK_RB_RIGHT_FRONT_PRELOAD_MM   5.0f // 在 LB 起摆前施加到 RF 的反向预加载量，并保持至 RB 落脚；正值 N 实际使 RF Y 减少 N mm。
...
#define DOG_GAIT_WALK_LIFT_END_PHASE             0.25f // 摆动前段结束相位：先以抬高为主，X 基本保持。
#define DOG_GAIT_WALK_TRANSFER_END_PHASE         0.50f // 摆动中段结束相位：在高位完成向前移动。
#define DOG_GAIT_WALK_TUCK_X_MM                  15.0f // 抬起初期主动后收量；先设为 0，仅观察逆运动学自然收腿效果。
...
static void DogGait_UpdateWalkFootTrajectories(void)
{
    for (uint8_t i = 0; i < DOG_GAIT_LEG_COUNT; i++)
    {
                         ...
                         DOG_GAIT_WALK_PHASE_BOUNDARY_EPSILON)) // 摆动相
        {
            ...
            /* 减速带开启统一模式时，前腿也使用抬起 -> 前移 -> 落下的三段式轨迹。 */
            if ((s_walk_front_rear_unified != 0U) ||
                (i == DOG_GAIT_LEG_LB) ||
                (i == DOG_GAIT_LEG_RB))
            {
                if (swing_phase < DOG_GAIT_WALK_LIFT_END_PHASE)
                { //在 0~0.25 阶段，主要是抬腿阶段，足端的 x 坐标会向后收缩，y 坐标会向上抬高。
                    float lift_phase =
                        swing_phase / DOG_GAIT_WALK_LIFT_END_PHASE;
                    float lift_smooth = DogGait_SmoothStep(lift_phase);

                    s_walk_foot_x[i] =
                        s_walk_swing_start_x[i] -
                        DOG_GAIT_WALK_TUCK_X_MM * lift_smooth; // x 向后收 DOG_GAIT_WALK_TUCK_X_MM = 15mm
                    s_walk_foot_y[i] =
                        support_height +
                        s_walk_step_height_mm * lift_smooth; // y 在 support_height 的基础上抬高 s_walk_step_height_mm
                }
                else if (swing_phase < DOG_GAIT_WALK_TRANSFER_END_PHASE)
                {
                    float transfer_phase =
                        (swing_phase - DOG_GAIT_WALK_LIFT_END_PHASE) /
                        (DOG_GAIT_WALK_TRANSFER_END_PHASE -
                         DOG_GAIT_WALK_LIFT_END_PHASE);
                ...
                {
                    float landing_phase =
                        (swing_phase - DOG_GAIT_WALK_TRANSFER_END_PHASE) /
...
static float DogGait_GetWalkBodyTarget(uint8_t active_leg, float pitch_deg)
{
    ...
    target = s_walk_cg_base_x_mm +
             ...
             (reference_phase_adjust * s_walk_phase_cg_gain +
              pitch_adjust);
...
static void DogGait_UpdateRearPreloadState(void)
{
    ...
    if ((s_walk_rb_preload_state == DOG_GAIT_RB_PRELOAD_NONE) &&
        (s_walk_phase >= (DOG_GAIT_WALK_PHASE_PER_LEG * 2.0f)) &&
         ...
         s_walk_second_front_to_rear_hold_limit))
    {
         ...
         * allowing LB to swing.  They remain active through RB's landing. */
        s_walk_rb_preload_state = DOG_GAIT_RB_PRELOAD_HOLD;
        s_walk_rb_preload_stable_updates = 0U;
        s_walk_rb_preload_rb_swing_started = 0U;
        s_walk_rb_preload_rb_touchdown_output_done = 0U;
...
{
    DogGait_InitLeg(&s_gait[DOG_GAIT_LEG_LF], DOG_GAIT_DEFAULT_H_MM, DOG_GAIT_DEFAULT_R_MM, DOG_GAIT_DEFAULT_L1_MM, DOG_GAIT_DEFAULT_L2_MM, DOG_GAIT_DEFAULT_BIAS_ANGLE_DEG);
    DogGait_InitLeg(&s_gait[DOG_GAIT_LEG_RF], DOG_GAIT_DEFAULT_H_MM, DOG_GAIT_DEFAULT_R_MM, DOG_GAIT_DEFAULT_L1_MM, DOG_GAIT_DEFAULT_L2_MM, DOG_GAIT_DEFAULT_BIAS_ANGLE_DEG);
    DogGait_InitLeg(&s_gait[DOG_GAIT_LEG_LB], DOG_GAIT_DEFAULT_H_MM, DOG_GAIT_DEFAULT_R_MM, DOG_GAIT_DEFAULT_L1_MM, DOG_GAIT_DEFAULT_L2_MM, DOG_GAIT_DEFAULT_BIAS_ANGLE_DEG);
    DogGait_InitLeg(&s_gait[DOG_GAIT_LEG_RB], DOG_GAIT_DEFAULT_H_MM, DOG_GAIT_DEFAULT_R_MM, DOG_GAIT_DEFAULT_L1_MM, DOG_GAIT_DEFAULT_L2_MM, DOG_GAIT_DEFAULT_BIAS_ANGLE_DEG);
...
/*
 * 名称：DogGait_SetWalkFrontRearUnified
 * 作用：设置前后腿摆动轨迹是否统一为三段式。
 * 输入：enabled 1 表示前腿与后腿轨迹一致；0 表示前腿保持正弦、后腿三段式。
 * 输出：无返回值，更新 walk 轨迹模式。
 */
void DogGait_SetWalkFrontRearUnified(uint8_t enabled)
{
    s_walk_front_rear_unified = (enabled != 0U) ? 1U : 0U;
}

/*
 * 名称：DogGait_SetWalkRbPreloadStableUpdates
 * 作用：设置 RB 起摆前预加载稳定的更新次数；0 表示减速带关闭该停顿。
 * 输入：updates 保持的更新周期数，每个更新周期约 100 ms。
 * 输出：无返回值，更新 walk 预加载稳定参数。
 */
 ...
 * 输入：phase_cg_gain 缩放系数；1.0 保持原有幅度，0.0 关闭动态切换。
 * 输出：无返回值，更新 walk 动态重心缩放参数。
 */
void DogGait_SetWalkPhaseCgGain(float phase_cg_gain)
{
    s_walk_phase_cg_gain = DogGait_ClampFloat(phase_cg_gain, 0.0f, 3.0f);
}
    ...
    */
    ...
    if (fabsf(s_walk_body_x_goal_mm - s_walk_body_x_state_mm) < DOG_GAIT_WALK_BODY_READY_MM)
    {
            ...
            DOG_GAIT_SECOND_FRONT_TO_REAR_HOLD)
        {
            ...
            if (s_walk_rb_preload_state == DOG_GAIT_RB_PRELOAD_HOLD)
            {
                /* The body transfer has finished.  Keep LB as a support
                 * leg while the LF/RF preload settles, so the preload does
                 * not fight the X-direction body transfer. */
                if (s_walk_rb_preload_stable_updates <
                    s_walk_rb_preload_stable_limit)
                {
            ...
            /* The preload is established.  The next call releases LB and
             * begins its swing while retaining the preload through RB's
             * subsequent swing and landing. */
            s_walk_second_front_to_rear_state =
                DOG_GAIT_SECOND_FRONT_TO_REAR_SWING;
            return;
        }
