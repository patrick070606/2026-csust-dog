...
#define DOG_GAIT_STAIR_MAX_X_OFFSET_MM       60.0f // 上台阶时足端在x轴方向相对于站立基准位置的最大偏移，单位毫米，根据实际情况调整，过大会导致步态不够稳定。
#define DOG_GAIT_STAIR_MAX_Y_OFFSET_MM       50.0f // 上台阶时足端在y轴方向相对于站立基准位置的最大偏移，单位毫米，根据实际情况调整，过大会导致步态不够稳定。
