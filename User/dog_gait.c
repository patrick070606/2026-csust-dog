...
#define DOG_GAIT_WALK_RB_LEFT_REAR_PRELOAD_MM    -12.0f // LB 起摆前开始施加到 LB，并连续保持至 RB 落脚后的预加载量。
#define DOG_GAIT_WALK_RB_RIGHT_FRONT_PRELOAD_MM   1.0f // 在 LB 起摆前施加到 RF 的反向预加载量，并保持至 RB 落脚；正值 N 实际使 RF Y 减少 N mm。
#define DOG_GAIT_WALK_RB_EXTRA_LEFT_FRONT_PRELOAD_MM  -7.0f // RB 起摆前额外叠加到 LF 的预加载量。
#define DOG_GAIT_WALK_RB_EXTRA_RIGHT_FRONT_PRELOAD_MM  7.0f // RB 起摆前额外叠加到 RF 的反向预加载量。
...
/* Keep a separate step length for every leg.  The public side API assigns
 * the same value to LF/LB and RF/RB, while retaining the option to extend
 * this to individual-leg tuning later. */
                ...
                (i == DOG_GAIT_LEG_RB))// 如果是两条后腿，就采用抬起 - > 前移 - > 落下的三阶段方式来计算足端轨迹。
            {
                ...
                else if (swing_phase < DOG_GAIT_WALK_REAR_TRANSFER_END_PHASE) 
                {
                    ...
                    s_walk_foot_x[i] =
                        ...
                        (step_length_mm + DOG_GAIT_WALK_REAR_TUCK_X_MM) *
