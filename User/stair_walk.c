...
#define STAIR_WALK_TEST_STEP_H_MM             80.0f // 爬楼梯时的最大抬腿高度，单位 mm；较高的抬腿用于跨过台阶边缘。
...
#define STAIR_WALK_MIN_CYCLES                 2U    // 至少完成 2 个完整行走周期后，才允许判断爬楼梯已经结束，防止刚启动时误判。
#define STAIR_WALK_LEVEL_PITCH_DEG            5.0f  // “机身恢复水平”的俯仰角阈值：滤波后 |pitch| 不得超过 5°。
#define STAIR_WALK_LEVEL_ROLL_DEG             6.0f  // “机身恢复水平”的横滚角阈值：滤波后 |roll| 不得超过 6°。
#define STAIR_WALK_LEVEL_STABLE_MS            800U  // pitch 和 roll 同时满足水平阈值后，必须连续稳定 800 ms 才判定爬楼梯完成。
