...
/* 舵机旋转测试入口：上电后只初始化并旋转一次投掷舵机，不启动步态和循迹。 */
void DogTask_ServoRotationTest_Init(void)
{
    ThrowServo_Init();
    ThrowServo_Start(THROW_SERVO_DIRECTION_CW);
}

/* 舵机旋转测试运行：只推进投掷舵机状态，旋转完成后保持停止。 */
void DogTask_ServoRotationTest_Run(void)
{
    ThrowServo_Update();
}
