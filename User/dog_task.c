...
//0 LF 1 RF 2 LB 3 RB
float DOG_TASK_SHIFT_R_MML[4] = {29.0f, -7.0f, 29.0f, -7.0f}; // 表示机器人步态的前进半径，单位毫米。四条腿的前进半径可以不同，这里设置为相同的值。
float DOG_TASK_SHIFT_R_MMR[4] = {-14.0f, 22.0f, -14.0f, 22.0f}; // 表示机器人步态的前进半径，单位毫米。四条腿的前进半径可以不同，这里设置为相同的值。
...
static void DogTask_ApplyMotion(DogTaskMotion_t motion)
{
    ...
    {
        DogGait_SetShiftLeftParams(DOG_TASK_SHIFT_H_MM,
    ...
    {
        DogGait_SetShiftRightParams(DOG_TASK_SHIFT_H_MM,
...
void DogTask_Run(void)
{
    ...
    DogGait_SetShiftLeftParams(DOG_TASK_SHIFT_H_MM, DOG_TASK_SHIFT_R_MML, 0.25);
